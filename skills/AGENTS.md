...
정식 배포명은 `engram-ms` (레거시 별칭 `memorytrace` 도 동일 엔진).
...
**명령어**
```bash
engram init                    # DB 초기화 (최초 1회, ~/.engram/memory.db)
...
**고급 CLI / SDK**
- `engram-advanced --db <path> search "..." --max-results 5` / `engram-advanced --json health` — 커스텀 DB 경로나 구조화된 출력이 필요할 때
- Python SDK: `from engram.integrations.sdk import EngramSDK`

**선택 기능** (필요할 때만):
`pip install "engram-ms[llm|semantic|mcp|full]"`
